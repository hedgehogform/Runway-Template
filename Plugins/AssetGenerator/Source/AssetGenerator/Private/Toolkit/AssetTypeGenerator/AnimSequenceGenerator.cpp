...
{
	UClass *ReimportFbxAnimSequenceFactoryClass = FindObject<UClass>(NULL, TEXT("/Script/UnrealEd.ReimportFbxAnimSequenceFactory"));
	if (!ReimportFbxAnimSequenceFactoryClass)
	{
		UE_LOG(LogAssetGenerator, Error, TEXT("Failed to find ReimportFbxAnimSequenceFactory class"));
		return;
	}
...
bool UAnimSequenceGenerator::IsAnimationSourceUpToDate(UAnimSequence *Asset) const
{
	...
	{
		UE_LOG(LogAssetGenerator, Warning, TEXT("AnimSequence %s has no source files in AssetImportData"), *GetPackageName().ToString());
		return false;
	}
