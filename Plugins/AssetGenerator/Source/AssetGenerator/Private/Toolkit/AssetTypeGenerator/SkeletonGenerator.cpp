...
bool FSkeletonCompareData::IsSkeletonUpToDate(USkeleton *Skeleton) const
{
	...
	for (const TPair<FName, FReferencePose> &NewReferencePose : AnimRetargetSources)
	{
		...
		FReferencePose *ExistingPosePtr = Skeleton->AnimRetargetSources.Find(NewPose.PoseName);
		if (!ExistingPosePtr)
		{
			UE_LOG(LogAssetGenerator, Warning, TEXT("AnimRetargetSource not found: %s"), *NewPose.PoseName.ToString());
			return false;
		}
		FReferencePose &ExistingPose = *ExistingPosePtr;
