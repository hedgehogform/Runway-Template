...
void UUserDefinedStructGenerator::OnExistingPackageLoaded()
{
	...
	if (!IsStructUpToDate(ExistingStruct))
	{
		...
		if (ExistingStruct->EditorData)
		{
			UUserDefinedStructEditorData *EditorData = Cast<UUserDefinedStructEditorData>(ExistingStruct->EditorData);
			if (EditorData)
			{
			...
			{
				UE_LOG(LogAssetGenerator, Warning, TEXT("EditorData is not UUserDefinedStructEditorData for struct %s"), *ExistingStruct->GetPathName());
			}
		}
