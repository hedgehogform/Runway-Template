...
void UStaticMeshGenerator::PopulateStaticMeshWithData(UStaticMesh *Asset)
{
	...
	{
		UE_LOG(LogAssetGenerator, Error, TEXT("AssetObjectData field is missing or invalid in StaticMesh"));
		return;
	}
	...
	for (int32 i = 0; i < FMath::Min(Materials.Num(), Asset->StaticMaterials.Num()); i++)
	{
		...
		if (MaterialInterface)
		{
			...
			{
				UE_LOG(LogAssetGenerator, Warning, TEXT("Material interface cast failed for static mesh material slot %d"), i);
			}
		}
	}
...
bool UStaticMeshGenerator::IsStaticMeshDataUpToDate(UStaticMesh *Asset) const
{
	...
	if (!AssetObjectProperties.IsValid())
	{
		UE_LOG(LogAssetGenerator, Error, TEXT("AssetObjectData field is missing or invalid in IsStaticMeshDataUpToDate"));
		return false;
	}
