...
void USkeletalMeshGenerator::SetupFbxImportSettings(UFbxImportUI *ImportUI, const FName &AssetName, UPackage *Package) const
{
	...
	if (!IsGeneratingPublicProject())
	{
		...
		if (AssetObjectData.IsValid() && AssetObjectData->HasField(TEXT("Skeleton")))
		{
			const int32 SkeletonObjectIndex = AssetObjectData->GetIntegerField(TEXT("Skeleton"));
		...
		{
			UE_LOG(LogAssetGenerator, Warning, TEXT("No skeleton data found for SkeletalMesh %s"), *GetPackageName().ToString());
			ImportUI->Skeleton = FPublicProjectStubHelper::DefaultSkeletalMeshSkeleton.GetObject();
		}
	}
	else
...
void USkeletalMeshGenerator::PopulateSkeletalMeshProperties(USkeletalMesh *Asset)
{
	...
	for (int32 i = 0; i < FMath::Min(Materials.Num(), Asset->Materials.Num()); i++)
	{
		...
		{
			UMaterialInterface *CastedMaterial = Cast<UMaterialInterface>(MaterialInterface);
			if (CastedMaterial)
			{
				StaticMaterial.MaterialInterface = CastedMaterial;
			}
			else
			{
				UE_LOG(LogAssetGenerator, Warning, TEXT("Material interface cast failed for skeletal mesh material slot %d"), i);
			}
		}
	}
	...
	if (Asset->bEnablePerPolyCollision)
	{
		...
		if (BodySetupObject)
		{
			UBodySetup *CastedBodySetup = Cast<UBodySetup>(BodySetupObject);
			if (CastedBodySetup)
			{
				Asset->BodySetup = CastedBodySetup;
			}
			else
			{
				UE_LOG(LogAssetGenerator, Warning, TEXT("BodySetup cast failed for skeletal mesh"));
			}
		}
	}
	MarkAssetChanged();
...
bool USkeletalMeshGenerator::IsSkeletalMeshSourceFileUpToDate(USkeletalMesh *Asset) const
{
	...
	{
		UE_LOG(LogAssetGenerator, Warning, TEXT("SkeletalMesh %s has no source files in AssetImportData"), *GetPackageName().ToString());
		return false;
	}
