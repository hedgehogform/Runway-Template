...
void UDataTableGenerator::CreateAssetPackage()
{
	...
	UObject *RowStructObject = GetObjectSerializer()->DeserializeObject(RowStructIndex);
	if (RowStructObject)
	{
		UScriptStruct *RowStruct = Cast<UScriptStruct>(RowStructObject);
		if (RowStruct)
		{
		...
		{
			UE_LOG(LogAssetGenerator, Error, TEXT("Failed to cast RowStruct for DataTable %s"), *GetPackageName().ToString());
		}
	}
	else
	{
		UE_LOG(LogAssetGenerator, Error, TEXT("Failed to deserialize RowStruct for DataTable %s"), *GetPackageName().ToString());
	}
}
...
void UDataTableGenerator::PopulateAssetWithData()
{
	...
	if (!RowData.IsValid())
	{
		UE_LOG(LogAssetGenerator, Error, TEXT("RowData field is missing or invalid in DataTable"));
		return;
	}
