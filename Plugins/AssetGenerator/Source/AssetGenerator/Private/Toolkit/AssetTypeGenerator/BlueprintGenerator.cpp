...
void UBlueprintGenerator::PostConstructOrUpdateAsset(UBlueprint *Blueprint)
{
	...
	for (int32 i = 0; i < ImplementedInterfaces.Num(); i++)
	{
		...
		{
			UE_LOG(LogAssetGenerator, Warning, TEXT("Failed to deserialize interface class for Blueprint %s"), *GetPackageName().ToString());
		}
	}
...
void UBlueprintGenerator::PopulateAssetWithData()
{
	...
	const bool bChangedFunctions = FBlueprintGeneratorUtils::CreateNewBlueprintFunctions(Blueprint, Functions, [&](const FDeserializedFunction &Function)
																						 { return true; }, true);
...
void UBlueprintGenerator::FinalizeAssetCDO()
{
	...
	if (bScriptObjectChanged && SimpleConstructionScriptIndex != INDEX_NONE)
	{
		...
		{
			UE_LOG(LogAssetGenerator, Error, TEXT("Failed to deserialize SimpleConstructionScript for Blueprint %s"), *GetPackageName().ToString());
			return;
		}
		...
		if (!SCS)
		{
			UE_LOG(LogAssetGenerator, Error, TEXT("Failed to cast SimpleConstructionScript for Blueprint %s"), *GetPackageName().ToString());
			return;
		}
