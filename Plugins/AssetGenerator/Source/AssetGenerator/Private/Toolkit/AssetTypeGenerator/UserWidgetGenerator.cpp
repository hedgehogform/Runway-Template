...
void UUserWidgetGenerator::FinalizeAssetCDO()
{
	...
	if (!AssetObjectData.IsValid())
	{
		UE_LOG(LogAssetGenerator, Error, TEXT("AssetObjectData field is missing or invalid in UserWidget"));
		return;
	}
	...
	if (bWidgetTreeChanged)
	{
		...
		UWidgetTree *CastedWidgetTree = Cast<UWidgetTree>(DuplicatedWidgetTree);
		if (CastedWidgetTree)
		{
			WidgetBlueprint->WidgetTree = CastedWidgetTree;
		}
		else
		{
			UE_LOG(LogAssetGenerator, Error, TEXT("Failed to cast DuplicatedWidgetTree to UWidgetTree"));
			return;
		}
